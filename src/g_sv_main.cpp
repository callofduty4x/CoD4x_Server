...
int BG_GetPerkIndexForName(const char* name)
{
    ...
    {
        "specialty_gpsjammer", "specialty_bulletaccuracy", "specialty_fastreload", "specialty_rof",
        ...
        "specialty_armorvest", "specialty_fraggrenade", "specialty_specialgrenade"
    };

    int len = static_cast<int>(sizeof(perks)/sizeof(perks[0]));
    if(!name)
        return len;
...
void Helicopter_RegisterCvars()
{
     ...
     vehHelicopterSoftCollisions = Cvar_RegisterBool( "vehHelicopterSoftCollisions", qfalse, 0x80u, "Player helicopters have soft collisions (slow down before they collide).");
     ...
     vehHelicopterInvertUpDown = Cvar_RegisterBool( "vehHelicopterInvertUpDown", qfalse, 0x80u, "Invert the altitude control on the player helicopter.");
...
{
    vehDebugServer = Cvar_RegisterBool("vehDebugServer", qfalse, 0x80u, "Turn on debug information for vehicles");
...
{
    missileDebugDraw = Cvar_RegisterBool("missileDebugDraw", qfalse, 0, "Draw guided missile trajectories.");
    missileDebugText = Cvar_RegisterBool("missileDebugText", qfalse, 0, "Print debug missile info to console.");
    missileDebugAttractors = Cvar_RegisterBool( "missileDebugAttractors", qfalse, 0, "Draw the attractors and repulsors. Attractors are green, and repulsors are yellow.");
}
...
void BG_RegisterCvars()
{
    ...
    inertiaDebug = Cvar_RegisterBool("inertiaDebug", qfalse, 0x180u, "Show inertia debug information");
    ...
    player_scopeExitOnDamage = Cvar_RegisterBool( "player_scopeExitOnDamage", qfalse, 0x180u, "Exit the scope if the player takes damage");
    ...
    player_debugHealth = Cvar_RegisterBool( "player_debugHealth", qfalse, 0x180u, "Turn on debugging info for player health");
    player_sustainAmmo = Cvar_RegisterBool( "player_sustainAmmo", qfalse, 0x80u, "Firing weapon will not decrease clip ammo.");
    ...
    player_turnAnims = Cvar_RegisterBool( "player_turnAnims", qfalse, 0x180u, "Use animations to turn a player's model in multiplayer");
