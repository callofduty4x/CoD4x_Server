...
static int  CM_GridPlane( int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i, int j, int tri ) {
	...
	// should never happen
	Com_Printf( CON_CHANNEL_SYSTEM, "WARNING: CM_GridPlane unresolvable\n" );
	return -1;
}
...
static void CM_SetBorderInward( facet_t *facet, cGrid_t *grid, int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2],
	...
	for ( k = 0 ; k < facet->numBorders ; k++ ) {
		...
		if ( front && !back ) {
			...
			Com_DPrintf(CON_CHANNEL_SYSTEM, "WARNING: CM_SetBorderInward: mixed plane sides\n" );
			facet->borderInward[k] = qfalse;
			if ( !debugBlock ) {
				debugBlock = qtrue;
		...
		for ( dir = -1 ; dir <= 1 ; dir += 2, order++ )
		{
			...
			if ( i == facet->numBorders ) {
				if ( facet->numBorders > 4 + 6 + 16 ) {
					Com_Printf(CON_CHANNEL_SYSTEM, "ERROR: too many bevels\n" );
				}
				...
				if ( i == facet->numBorders ) {
					...
					for ( k = 0 ; k < facet->numBorders ; k++ ) {
						if ( facet->borderPlanes[facet->numBorders] ==
							...
							Com_Printf(CON_CHANNEL_SYSTEM, "WARNING: bevel plane already used\n" );
						}
					}
	...
	for ( i = 0, facet = pc->facets ; i < pc->numFacets ; i++, facet++ ) {
		...
		for ( k = 0 ; k < facet->numBorders + 1; k++ ) {
			...
			if ( w ) {
				...
				Com_Printf(CON_CHANNEL_SYSTEM, "winding chopped away by border planes\n" );
			}
		}
	}
