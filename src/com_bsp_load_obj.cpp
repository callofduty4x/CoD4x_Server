...
bool CCDECL Com_IsBspLoaded()
{
  return comBspGlob.header != 0;
}

unsigned int CCDECL Com_GetBspLumpCountForVersion(const int version)
{
...
bool CCDECL Com_CheckVersionLumpCountError(int version)
{
...
extern "C" void CCDECL Com_LoadBsp(const char *filename)
{
...
byte* CCDECL Com_ValidateBspLumpData(enum LumpType type, unsigned int offset, unsigned int length, unsigned int elemSize, unsigned int *count)
{
  assert( count );
...
byte *CCDECL Com_GetBspLump(enum LumpType type, unsigned int elemSize, unsigned int *count)
{
...
unsigned int CCDECL Com_GetBspVersion()
{
  assert(Com_IsBspLoaded());
...
bool CCDECL Com_BspHasLump(enum LumpType type)
{
  unsigned int count;
