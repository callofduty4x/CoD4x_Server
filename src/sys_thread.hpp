...
void CCDECL Sys_EnterCriticalSection(int section);
void CCDECL Sys_LeaveCriticalSection(int section);
void CCDECL Sys_EnterCriticalSectionInternal(int section);
void CCDECL Sys_LeaveCriticalSectionInternal(int section);
void CCDECL Sys_InitializeCriticalSections( void );
void CCDECL Sys_InitMainThread( void );
qboolean CCDECL Sys_IsMainThread( void );
qboolean CCDECL Sys_IsDatabaseThread( void );
qboolean CCDECL Sys_IsServerThread( void );
qboolean CCDECL Sys_IsRenderThread( void );
void Sys_SyncDatabase();
void Com_InitThreadData(int threadcontext);
void* CCDECL Sys_GetValue(int key);
void CCDECL Sys_SetValue(int key, void* value);
...
void CCDECL Sys_WaitStartDatabase();
qboolean CCDECL Sys_SpawnDatabaseThread(void (*db_proc)(unsigned int p));
void CCDECL Sys_DatabaseCompleted();
void Sys_WakeDatabase();
void Sys_WakeDatabase2();
void Sys_NotifyDatabase();
qboolean Sys_IsDatabaseReady();
qboolean CCDECL Sys_IsDatabaseReady2();
void CCDECL Sys_SuspendDatabaseThread(enum ThreadOwner owner);
qboolean CCDECL Sys_HaveSuspendedDatabaseThread(enum ThreadOwner to);
void CCDECL Sys_ResumeDatabaseThread(enum ThreadOwner to);
void CCDECL Sys_DatabaseCompleted2();


signed int CCDECL Sys_WaitForObject(HANDLE handle);
signed int CCDECL Sys_IsObjectSignaled(HANDLE handle);
